...
// EDIT for 40-pin device support: "portInputRegister(...)" referenced below is a standard Arduino macro that returns the corresponding port_to_input_PGM[] entry (PINA, etc.). 
// Its 0th entry is reserved (NOT_A_PORT); the rest refer to PORTA ~ PORTD respectively.
// The original PinChangeInt library ignores PORTA and creates PCintPort entries [0..2] corresponding to ports [B..D] respectively, and adjusts the portInputRegister index by +2
//   to skip both NOT_A_PORT and PORTA (the lower pin-count ATmegas do not pin out PORTA). For the larger devices We want PORTA usable, so adding PORTA as the 0th PCintPort e
//   entry and only adjusting by one.

#define FIRST_PORT_INPUT_OFFSET 1

class PCintPort {
public:
	PCintPort(int index,volatile uint8_t& maskReg) :
	portInputReg(*portInputRegister(index + FIRST_PORT_INPUT_OFFSET)),
	portPCMask(maskReg),
	PCICRbit(1 << index),
	firstPin(NULL)
	{
...
// NOTE: Again, for 40-pin support, defining 0 to be PORTA.

PCintPort portA=PCintPort(0,PCMSK0); // port PA==1  (from Arduino.h, Arduino version 1.0)

#ifndef NO_PORTB_PINCHANGES
PCintPort portB=PCintPort(1,PCMSK1); // port PB==2  (from Arduino.h, Arduino version 1.0)
#endif

#ifndef NO_PORTC_PINCHANGES
PCintPort portC=PCintPort(2,PCMSK2); // port PC==3  (also in pins_arduino.c, Arduino version 022)
#endif

#ifndef NO_PORTD_PINCHANGES
PCintPort portD=PCintPort(3,PCMSK3); // port PD==4
#endif
...
// Added PORTA case for midsize devices; fixed numbering of remaining cases
ISR(PCINT0_vect) {
	PCintPort::curr = portA.portInputReg; // version 1.6
	portA.PCint();
}

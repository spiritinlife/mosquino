...
// tg
//int main(void)
void main(void) __attribute__ ((noreturn)); // save a few bytes of stack ... where would we return to?
void main(void)
{
    uint8_t ch,ch2;
    uint16_t w;
...
//cleared by hardware four cycles after it is written or when CLKPS bits are written."
    ...
    WDTCSR |= _BV(WDCE) | _BV(WDE); // WDTCSR writes enabled for the next 4 cycles
    WDTCSR = 0;
